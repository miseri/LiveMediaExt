...
namespace lme
{
  ...
  const unsigned MAX_REVOLUTIONS = 15;
  const unsigned MAX_SAMPLES_TO_BE_PROCESSED_IN_EVENT_LOOP = 30;

  LiveSourceTaskScheduler0::LiveSourceTaskScheduler0(ChannelManager& channelManager)
    : m_uMaxDelayTime(100), // default time of once per second,
  ...
  void LiveSourceTaskScheduler0::processLiveSources()
  {
    ...
    // MAX_SAMPLES_TO_BE_PROCESSED_IN_EVENT_LOOP is reached. For the purpose fo the single
    // live media session this should suffice
    while (uiSessionCount < MAX_REVOLUTIONS)
    {
      for (MediaSessionMap_t::iterator it = m_mMediaSessions.begin();
        it != m_mMediaSessions.end() && uiSampleCount < MAX_SAMPLES_TO_BE_PROCESSED_IN_EVENT_LOOP;
        ++it)
      {
#ifdef DEBUG
...
#endif
            if (uiSampleCount >= MAX_SAMPLES_TO_BE_PROCESSED_IN_EVENT_LOOP)
            {
              // set exit condition of while
              uiSessionCount = MAX_REVOLUTIONS;
              break;
            }
          }
        }
...
#if 0
    VLOG(5) << "Processed " << uiSampleCount << " samples in event loop";
#endif
  }
